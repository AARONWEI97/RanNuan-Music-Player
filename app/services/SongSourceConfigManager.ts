...
export const SourceConfigManager = {
  ...
  async getConfig(songId: string | number): Promise<SongSourceConfig | null> {
    ...
    try {
      const sourcesJson = await AsyncStorage.getItem(key);
      const type = await AsyncStorage.getItem(key + TYPE_SUFFIX);
      if (sourcesJson) {
  ...
  async setConfig(songId: string | number, sources: string[], type: 'manual' | 'auto' = 'manual') {
    ...
    try {
      ...
      await AsyncStorage.setItem(key + TYPE_SUFFIX, type);
    } catch {}
  },
  ...
  async clearConfig(songId: string | number) {
    ...
    try {
      await AsyncStorage.multiRemove([key, key + TYPE_SUFFIX, key + TRIED_SUFFIX]);
    } catch {}
  },
