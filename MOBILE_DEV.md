...
| Phase 9 | 🚧 | **播放器迁移（expo-av → react-native-track-player）+ 灵动岛 + 后台播放** |
...
| 任务 | 说明 |
|------|------|
| 安装 react-native-track-player | `npx expo install react-native-track-player` |
| 从 metro.config.js BLOCKED_MODULES 移除 track-player | 允许打包 |
| 配置 app.json 音频后台模式 | `ios.infoPlist.UIBackgroundModes: ['audio']` |
| 创建 Development Build | `eas build --profile development --platform android/ios` |
| 验证 Development Build 可运行 | 在真机/模拟器安装测试 |

#### Step 2：trackPlayerService.ts 重写
...
| 事件监听 | PlaybackPlayWhenReady / PlaybackState / PlaybackProgressUpdated / PlaybackActiveTrackChanged |
| Now Playing Metadata | TrackPlayer.updateMetadataForTrack — 封面、标题、歌手 |
| 4 层结束检测简化 | TrackPlayer 内置事件可靠，可移除定时轮询 |

#### Step 3：usePlayer.ts 适配
...
| 自动下一首 | 监听 PlaybackState.Completed 事件替代 4 层检测 |
...
```typescript
// 在 App.tsx 或独立的 PlaybackService 中
useEffect(() => {
  ...
  const sub3 = TrackPlayer.addEventListener(Event.PlaybackActiveTrackChanged, ({ track }) => {
    if (!track) return;
    // 歌曲切换回调
  });

  // 自动下一首 — TrackPlayer 内置队列管理
  // 不再需要手动 4 层检测
