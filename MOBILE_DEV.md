...
| Phase 9 | 🚧 | **播放器迁移（expo-av → react-native-track-player）+ 灵动岛 + 后台播放 + 远程控制延迟修复 + 列表性能优化** |
...
// 4 层歌曲结束检测：didJustFinish + progress fallback + position>=duration + 定时轮询
function startEndCheck() {
  ...
  endCheckTimer = setInterval(async () => {
    ...
    if (!status.isLoaded) { hasEndedFlag = true; onPlaybackEndCallback?.(); stopEndCheck(); return; }
    if (status.positionMillis >= status.durationMillis - 200) {
      hasEndedFlag = true; onPlaybackEndCallback?.(); stopEndCheck();
    }
  }, 3000); // 前 80% 每 3 秒，进度回调自动提速到 1 秒
}
...
export async function playSong(song: SongResult, url: string): Promise<void> {
  ...
  const { sound } = await Audio.Sound.createAsync(
    { uri: url },
    { shouldPlay: true },
    handlePlaybackStatusUpdate, // 包含 didJustFinish 检测 + 进度回调 + 智能提速
  );
  soundRef = sound;
...
> ⚠️ **重要**：`onPlaybackEndCallback` 在 `usePlayer.ts` 中只注册一次（ref 模式），永不清理，避免多组件实例互相覆盖。详见 Bug 23。
...
## Phase 9：播放器迁移（expo-av → react-native-track-player）+ 灵动岛 + 后台播放 🚧 进行中
...
项目采用 **双服务架构** 处理 track-player 事件，确保前台 UI 和后台 headless 服务协调工作：

| 服务 | 文件 | 运行环境 | 职责 |
|------|------|---------|------|
| **PlaybackService** | `app/services/playbackService.ts` | Headless JS Task（后台） | **唯一的 play()/pause() 执行者**、状态日志 |
| **trackPlayerService** | `app/services/trackPlayerService.ts` | 主 App 进程 | 状态同步回调、切歌逻辑、播放结束检测、onReloadAndPlay |

**关键规则**：RemotePlay/RemotePause 事件会被两个服务同时接收。为避免竞态：
- PlaybackService 负责实际调用 `TrackPlayer.play()`/`pause()`
- trackPlayerService **不调用** play()/pause()，只负责状态同步和业务逻辑回调
- 两个服务通过回调函数（`onRemotePlayCallback`/`onRemotePauseCallback`）与 zustand store 通信

```
RemotePlay 事件 → PlaybackService: TrackPlayer.play()  [唯一执行者]
                → trackPlayerService: onRemotePlayCallback() → syncPlaybackState()  [状态同步]

RemotePause 事件 → PlaybackService: TrackPlayer.pause()  [唯一执行者]
                 → trackPlayerService: onRemotePauseCallback() → syncPlaybackState()  [状态同步]
```
...
## Phase 10：功能补齐 — MV 播放 + 播放历史 + 收藏 + 评论 + 歌单编辑 + 本地音乐 + 热力图 🚧 待开发
