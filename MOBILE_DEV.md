...
**13 个开发阶段已完成** ✅，**Phase 11 已实现 7/18 模块（核心功能覆盖完毕）**
...
| Phase 11 | 🚧 | **API 功能补齐 — 7/18 模块（用户/歌手/歌单/推荐/高级/评论 ✅，社区社交等后续按需）** |
| Phase 12 | ✅ | **播放模块终极修复（12项 — 音源解析、锁屏切歌、错误重试、双重触发等）** |
| Phase 13 | ✅ | **歌手详情页 UI 修复 + MV 播放音频冲突（4项 — 布局/滚动/3列/音频重叠）** |

> **当前版本可打包发布**。核心播放、登录、歌单、搜索、下载、评论、MV 等主要功能均已完善，剩余 API 模块（社区/电台/视频/云盘等 11 项）后续按需迭代。
...
│   │   │   ├── LyricView.tsx     # 歌词显示
│   │   │   └── LyricLine.tsx     # 单行歌词
│   │   ├── search/               # 搜索组件目录（功能已整合至 SearchScreen.tsx）
│   │   └── common/               # 其他通用组件
...
| 实现 SearchScreen | 搜索页 + 热搜 + 搜索历史 + 多 Tab 结果（单曲/歌手/专辑/歌单）+ 搜索结果分页加载 |
...
> Phase 11 期间对搜索模块进行了全面升级，解决历史搜索丢失、结果截断、miniPlayer 遮挡三大问题。

| 改进项 | 文件 | 说明 |
|--------|------|------|
| **历史搜索持久化** | `SearchScreen.tsx` | 改用 `AsyncStorage`（Key: `search_history`），历史搜索不再丢失，最多保存 20 条，支持单项点击/全部清除 |
| **搜索结果分页** | `SearchScreen.tsx` + `useSearch.ts` + `SongList.tsx` | 所有 4 个 Tab（单曲/歌手/专辑/歌单）均支持分页加载：首次加载 100 条，滑到底部自动触发 `onEndReached` 加载下一页，API `more` 字段驱动 |
| **底部安全区** | `SearchScreen.tsx` | `paddingBottom` 统一设为 `106`，搜索结果和热搜历史均不被 MiniPlayer 遮挡 |
| **SongList 扩展** | `SongList.tsx` | 新增 `onEndReached`/`onEndReachedThreshold`/`ListFooterComponent` prop |
| **useSearch 分页** | `useSearch.ts` | 新增 `loadMoreSongs()`/`hasMoreSong`/`loadingMore`，`SONG_PAGE_SIZE=100` |

**改动文件**：`app/screens/SearchScreen.tsx`、`app/hooks/useSearch.ts`、`app/components/music/SongList.tsx`
...
### 11.1 用户体系增强 🔴 高优先级 — ✅ 已完成 (2026-05-22)

> **全部 24/24 项已完成**。阶段二补齐了退出确认、账号展示、编辑资料、关注/粉丝列表、历史评论、歌单分离、社交状态等 10 项 UI 层功能。
...
#### 11.1.5 阶段二完成 (10/10) — 2026-05-22

| API | 接口地址 | 实现方式 | 状态 |
|-----|---------|---------|:---:|
| 退出登录确认弹窗 | — | `UserScreen`: `Alert.alert` 二次确认后调用 `handleLogout` | ✅ |
| 账号信息展示 | `GET /user/account` | `UserScreen` Hero 区域新增 VIP 标签 + 帐号信息行；`userStore.fetchAccountInfo()` | ✅ |
| 更新用户信息 | `POST /user/update` | 新建 `EditProfileScreen`：昵称/签名/性别/生日/省份/城市 表单，调用 `updateUserInfo` API | ✅ |
| 更新头像 | `POST /avatar/upload` | `EditProfileScreen` 完整集成 `expo-image-picker`（动态require + 权限 + 裁剪 + 上传） | ✅ |
| 用户关注列表 | `GET /user/follows` | 新建 `FollowListScreen`：通用关注/粉丝分页列表，支持关注/取消关注 | ✅ |
| 用户粉丝列表 | `GET /user/followeds` | 同 `FollowListScreen`，Tab 参数 `type='followeds'` | ✅ |
| 用户历史评论 | `GET /user/comment/history` | 新建 `CommentHistoryScreen`：分页展示评论历史、回复内容、点赞数 | ✅ |
| 用户状态展示 | `/user/social/status` × 4 | `UserScreen` Hero 区域新增社交状态行；`userStore.fetchSocialStatus()` | ✅ |
| 当前关注(新版) | `GET /user/follow/mixed` | `userStore.fetchFollowMixed()` API 已封装，供后续 UI 使用 | ✅ |
| 创建/收藏歌单分离 | `/user/playlist/create` `/collect` | `userStore` 新增 `fetchCreatedPlaylists`/`fetchCollectedPlaylists`，`UserScreen` 优先使用分离后的列表 | ✅ |

#### 11.1.6 阶段二改动文件清单

| 文件 | 改动类型 | 改动量 |
|------|---------|--------|
| `app/store/userStore.ts` | Store 增强 | +accountInfo/socialStatus/bindingInfo + 7 actions |
| `app/screens/UserScreen.tsx` | UI 增强 | 退出确认 + 账号/VIP/状态展示 + 粉丝/关注可点击 + 编辑入口 + 评论历史入口 + 歌单分离 |
| `app/screens/FollowListScreen.tsx` | **新建** | 关注/粉丝共用列表页，支持分页 + 关注/取关操作 |
| `app/screens/EditProfileScreen.tsx` | **新建** | 编辑资料页（昵称/签名/性别/生日/省份/城市），调用 updateUserInfo |
| `app/screens/CommentHistoryScreen.tsx` | **新建** | 历史评论列表页，分页展示评论/回复/点赞 |
| `app/types/navigation.ts` | 路由新增 | +FollowList / +EditProfile / +CommentHistory |
| `app/navigation/MainTabNavigator.tsx` | 路由注册 | 注册 3 个新页面
...
#### 11.1.7 跨模块联动修复（2026-05-22）

| 类别 | 问题 | 修复 |
|------|------|------|
| **登录 460** | 手机号/验证码/邮箱登录被网易风控拦截 `code: -460` | `app/api/login.ts`：所有登录 API 改为 GET + 添加 `randomCNIP=true` + `realIP=116.25.146.177`，phone/password 走 query string |
| **502 日志噪音** | `/user/social/status`、`/user/level`、`/user/account` 等服务端 502 打成 ERROR | `app/api/request.ts`：502 统一降级为 `console.warn`，标记 `[Upstream 502]` |
| **我喜欢的音乐** | 只拿歌曲 ID，名称/歌手/封面全是假的 | `LikedSongsScreen`：改为通过 `getUserPlaylist` 找到「我喜欢的音乐」歌单 → `getPlaylistTrackAll` 一次性加载所有歌曲详情 |
| **播放历史** | 本周/所有 Tab 切换 | `HistoryScreen`：去掉 Tab，只保留全部听歌数据 |
| **本地音乐** | 权限名称对不上 / 弹窗无按钮 | `LocalMusicScreen`：统一走 `MediaLibrary.requestPermissionsAsync()` 原生流程，所有错误路径带「去设置」按钮 |
| **热力图** | 三卡片数据每天一样（假分布到最近 5 天） | `HeatmapScreen`：统计改用真实 `playCount` 累加，卡片改为「总播放」「播放歌曲数」「平均每首」 |
| **VIP 展示** | 会员单独占一行 | `UserScreen`：VIP 移到 statsRow 等级后面，缩小 crown 图标 + 三档文字「黑胶VIP/VIP/普通」 |
| **等级进度条** | 底色太透 | `UserScreen`：`rgba(255,255,255,0.2)` → `0.4` |
| **专辑 Tab 歌手名** | 不显示 | `UserScreen`：API 返回 `artists` 数组 → `item.artists?.[0]?.name` |
| **歌单分离回退** | `/user/playlist/create` + `/collect` 返回格式不一致 | 恢复客户端过滤：`playlists.filter(p => !p.subscribed)` / `playlists.filter(p => p.subscribed)` |
| **全局 Alert 样式** | 待实现 | 后续作为独立任务：创建 `StyledAlert` 组件替代 `Alert.alert()` |
...
#### 11.3 实现总结 ✅ 已完成（2026-05-22）

> 补全 7 个歌单 API + 新建 2 个页面（歌单广场、每日推荐）

##### 新增 API (`app/api/playlist.ts`)

| API | 接口 | 说明 |
|-----|------|------|
| `getHotPlaylistCategories` | `GET /playlist/hot` | 热门歌单分类标签 |
| `getHighqualityTags` | `GET /playlist/highquality/tags` | 精品歌单标签 |
| `updatePlaylistOrder` | `POST /playlist/order/update` | 调整歌单歌曲顺序 |
| `getPlaylistSubscribers` | `GET /playlist/subscribers` | 收藏者列表（分页） |
| `getPlaylistDetailDynamic` | `GET /playlist/detail/dynamic` | 评论数/收藏数/播放数 |
| `updatePlaylistPlaycount` | `POST /playlist/update/playcount` | 更新播放计数 |
| `getPlaylistRcmd` | `GET /playlist/detail/rcmd/get` | 相关歌单推荐 |

##### 新增页面

| 页面 | 文件 | 功能 |
|------|------|------|
| **歌单广场** | `app/screens/PlaylistSquareScreen.tsx` | 按分类浏览全网歌单，热门标签 + 3列网格 + 无限滚动 |
| **每日推荐** | `app/screens/DailyRecommendScreen.tsx` | 每日30首推荐歌曲详情页，封面大图 + Hero 头部 + 播放全部 |

##### 入口改动

| 文件 | 改动 |
|------|------|
| `HomeScreen.tsx` | 快捷卡片「歌单广场」→ `PlaylistSquare`；「每日推荐」→ `DailyRecommend`；推荐歌单「更多」→ `PlaylistSquare` |
| `MainTabNavigator.tsx` | 注册 `PlaylistSquare` / `DailyRecommend` 路由 |
| `navigation.ts` | 新增路由类型定义 |
| `api/index.ts` | 补全 16 个 playlist API 导出 |
...
### 11.5 相似推荐 🟡 中优先级 — ✅ 已完成 (2026-05-22) — 5/5
...
**实现方式**：
- **API 层**：新建 `app/api/simi.ts`，5 个 GET 函数，全部免登
- **通用页面**：新建 `SimilarScreen`（`route.params: { id, type }`），根据 type 自动调用对应 API 并渲染对应卡片布局
  - 歌曲 → 列表行 + 可点击播放
  - 歌单/MV/歌手 → 3 列网格卡片
  - 用户 → 头像 + 昵称列表
- **入口**：歌曲三点菜单新增「相似推荐」→ `SimilarScreen(type='song')`
- **已有功能**：`ArtistDetailScreen` 内置了水平滚动相似歌手，无需额外入口

### 11.6 每日/个性推荐增强 🟡 中优先级 — ✅ 已完成 (2026-05-22) — 8/8
...
| 私人FM | `GET /personal_fm` | 私人FM模式（已有入口） | ✅ |
| FM模式选择 | `GET /personal/fm/mode` | aidj/DEFAULT/FAMILIAR/EXPLORE/SCENE_RCMD | ✅ |
| FM垃圾桶 | `POST /fm_trash` | 从FM移入垃圾桶(id) | ✅ |

**改动**：
- `app/api/music.ts`：补 `fmTrash`、`setFmMode`、`getRecommendResource`（其中 `fmTrash` 之前导出了但未定义，属于 bug）
- `app/screens/DailyRecommendScreen.tsx`：新增历史日推日期选择器（可展开/收起 date chip 列表）、三点菜单加「不感兴趣」按钮、切换历史日推后显示「回到今日」入口

---

### 🔧 跨模块增强 — 音源重新解析（移植桌面端）— 2026-05-22

> 从桌面端 RanNuan-Music-Play 移植「重新解析」功能：用户可手动选择 Q音/咪咕/酷狗/酷我/波动/GD音乐等音源重新解析当前歌曲，解决货不对板问题。

| 文件 | 类型 | 说明 |
|------|------|------|
| `app/services/SongSourceConfigManager.ts` | **新建** | 音源配置管理器（AsyncStorage 持久化），支持 manual/auto 模式、已尝试源追踪 |
| `app/components/player/SourcePickerModal.tsx` | **新建** | 底部弹窗，7 个音源选择（图标+名称+颜色），当前选中高亮 ✓，底部恢复默认 |
| `app/hooks/useReparse.ts` | **新建** | 封装「打开弹窗 → 选音源 → 清缓存 → 强制解析 → 播放」流程 |
| `app/services/musicParserService.ts` | 增强 | +`parseMusicWithSource()` 强制指定源解析；+`UnblockApiMatchStrategy.parseWithSource()` |
| `app/hooks/useSongActionSheet.ts` | 增强 | 三点菜单 +「选择播放音源」（仅当传入 `onReparse` 回调时显示） |
| SearchScreen 等 7 个页面 | 集成 | 全部接入 `useReparse` + `SourcePickerModal` |

### 🔧 跨模块增强 — 下载体验优化 — 2026-05-22

| 改进 | 详情 |
|------|------|
| **下载 toast 加导航** | 点击下载 →「已加入下载」+ 歌曲名 + **「查看下载」按钮** → 跳转下载管理页 |
| **完成/失败通知** | `downloadService` 完成/失败时调 `showDownloadSuccess`/`showDownloadError`（之前定义了但从未调用） |
| **下载中 badge** | UserScreen「下载管理」卡片右上角红色数字 badge，实时显示下载中任务数 |

### 11.7 评论增强 🟢 低优先级 — ✅ 已完成 (2026-05-22)

> **全部 8/8 API 已封装**。`app/api/comment.ts` 新增 7 个函数：新版评论接口、视频评论、DJ评论、批量统计、举报、抱一抱、抱一抱列表。

| API | 接口地址 | 函数名 | 功能说明 | 需要登录 | 状态 |
|-----|---------|--------|---------|:---:|:---:|
| 楼层评论 | `GET /comment/floor` | `getFloorComment({parentCommentId,id,type,limit})` | 子评论（Phase 10 已完成） | ❌ | ✅ |
| 新版评论接口 | `GET /comment/new` | `getNewComment({id,type,pageNo,pageSize,sortType,cursor})` | 分页+排序(推荐/热度/时间) | ❌ | ✅ |
| 视频评论 | `GET /comment/video` | `getVideoComment({id,limit,offset,before})` | 视频评论区 | ❌ | ✅ |
| 电台节目评论 | `GET /comment/dj` | `getDjComment({id,limit,offset,before})` | DJ节目评论区 | ❌ | ✅ |
| 批量评论统计 | `GET /comment/info/list` | `getCommentInfoList(type, ids)` | 多资源评论数 | ❌ | ✅ |
| 举报评论 | `POST /comment/report` | `reportComment({id,cid,reason,type})` | 举报 | ✅ | ✅ |
| 抱一抱评论 | `POST /hug/comment` | `hugComment({uid,cid,sid})` | 抱评论 | ✅ | ✅ |
| 抱一抱列表 | `GET /comment/hug/list` | `getCommentHugList({uid,cid,sid,limit,offset})` | 评论抱抱列表 | ❌ | ✅ |

**改动文件**：`app/api/comment.ts` (+7 函数)、`app/api/index.ts` (+7 导出)
...
### 11.14 高级功能 🟢 低优先级 — ✅ 已完成 (2026-05-22)

> **全部 36/36 API 已封装**。新建 `app/api/style.ts`（曲风 7）、`app/api/voice.ts`（播客 10）、`app/api/advanced.ts`（高级 9），扩展 `app/api/music.ts`（歌曲增强 12）。

| API | 接口地址 | 实现文件 | 函数名 | 需要登录 | 状态 |
|-----|---------|---------|--------|:---:|:---:|
| 音乐百科 | `GET /song/wiki/summary` | `api/music.ts` | `getSongWiki(id)` | ❌ | ✅ |
| 歌曲创作者 | `GET /song/creators` | `api/music.ts` | `getSongCreators(id)` | ❌ | ✅ |
| 歌曲动态封面 | `GET /song/dynamic/cover` | `api/music.ts` | `getSongDynamicCover(id)` | ✅ | ✅ |
| 副歌时间 | `GET /song/chorus` | `api/music.ts` | `getSongChorus(id)` | ❌ | ✅ |
| 歌曲音质详情 | `GET /song/music/detail` | `api/music.ts` | `getMusicQualityDetail(id)` | ❌ | ✅ |
| 灰色歌曲版本推荐 | `GET /song/copyright/rcmd` | `api/music.ts` | `getSongCopyrightRcmd(songid)` | ❌ | ✅ |
| 歌曲红心数量 | `GET /song/red/count` | `api/music.ts` | `getSongRedCount(id)` | ❌ | ✅ |
| 歌曲是否喜爱 | `GET /song/like/check` | `api/music.ts` | `checkSongLike(ids)` | ✅ | ✅ |
| 客户端下载新版 | `GET /song/download/url/v1` | `api/music.ts` | `getSongDownloadUrl(id, level, encodeType)` | ❌ | ✅ |
| 灰歌解灰 | `GET /song/url/match` | `api/music.ts` | `matchSongUrl(id, source)` | ❌ | ✅ |
| 歌曲是否可用 | `GET /check/music` | `api/advanced.ts` | `checkMusic(id, br)` | ❌ | ✅ |
| 乐谱列表 | `GET /sheet/list` | `api/music.ts` | `getSheetList(id)` | ❌ | ✅ |
| 乐谱内容 | `GET /sheet/preview` | `api/music.ts` | `getSheetPreview(id)` | ✅ | ✅ |
| 曲风列表 | `GET /style/list` | `api/style.ts` | `getStyleList()` | ❌ | ✅ |
| 曲风偏好 | `GET /style/preference` | `api/style.ts` | `getStylePreference()` | ✅ | ✅ |
| 曲风详情 | `GET /style/detail` | `api/style.ts` | `getStyleDetail(tagId)` | ❌ | ✅ |
| 曲风-歌曲 | `GET /style/song` | `api/style.ts` | `getStyleSong(tagId)` | ❌ | ✅ |
| 曲风-专辑 | `GET /style/album` | `api/style.ts` | `getStyleAlbum(tagId)` | ❌ | ✅ |
| 曲风-歌单 | `GET /style/playlist` | `api/style.ts` | `getStylePlaylist(tagId)` | ❌ | ✅ |
| 曲风-歌手 | `GET /style/artist` | `api/style.ts` | `getStyleArtist(tagId)` | ❌ | ✅ |
| 回忆坐标 | `GET /music/first/listen/info` | `api/advanced.ts` | `getFirstListenInfo(id)` | ❌ | ✅ |
| 年度听歌报告 | `GET /summary/annual` | `api/advanced.ts` | `getAnnualSummary(year)` | ✅ | ✅ |
| 音乐日历 | `GET /calendar` | `api/advanced.ts` | `getMusicCalendar(startTime, endTime)` | ✅ | ✅ |
| 私人DJ | `GET /aidj/content/rcmd` | `api/advanced.ts` | `getAidjContentRcmd(lng, lat)` | ❌ | ✅ |
| 跑步漫游 | `GET /radio/sport/get` | `api/advanced.ts` | `getRadioSport(bpm)` | ❌ | ✅ |
| 听歌识曲 | `POST /audio/match` | `api/advanced.ts` | `matchAudio(duration, audioFP)` | ❌ | ✅ |
| 热门歌手 | `GET /top/artists` | `api/advanced.ts` | `getTopArtists({type, limit, offset})` | ❌ | ✅ |
| 歌手榜 | `GET /toplist/artist` | `api/advanced.ts` | `getToplistArtist(type)` | ❌ | ✅ |
| 播客搜索 | `GET /voicelist/search` | `api/voice.ts` | `searchVoiceList(keyword, limit, offset)` | ❌ | ✅ |
| 播客列表 | `GET /voicelist/list` | `api/voice.ts` | `getVoiceList(voiceListId, limit, offset)` | ❌ | ✅ |
| 播客声音搜索 | `GET /voicelist/list/search` | `api/voice.ts` | `searchVoiceInList(params)` | ❌ | ✅ |
| 播客声音详情 | `GET /voice/detail` | `api/voice.ts` | `getVoiceDetail(id)` | ❌ | ✅ |
| 播客声音排序 | `POST /voicelist/trans` | `api/voice.ts` | `updateVoiceOrder(voicelistId, ids)` | ✅ | ✅ |
| 播客列表详情 | `GET /voicelist/detail` | `api/voice.ts` | `getVoiceListDetail(id)` | ❌ | ✅ |
| 播客删除 | `POST /voice/delete` | `api/voice.ts` | `deleteVoice(ids)` | ✅ | ✅ |
| 播客上传声音 | `POST /voice/upload` | `api/voice.ts` | `uploadVoice(formData)` | ✅ | ✅ |
| 我创建的播客 | `GET /voicelist/my/created` | `api/voice.ts` | `getMyCreatedVoiceList()` | ✅ | ✅ |
| 声音歌词 | `GET /voice/lyric` | `api/voice.ts` | `getVoiceLyric(id)` | ❌ | ✅ |

#### 改动文件清单

| 文件 | 改动类型 | 改动量 |
|------|---------|--------|
| `app/api/style.ts` | **新建** | +7 函数 |
| `app/api/voice.ts` | **新建** | +10 函数 |
| `app/api/advanced.ts` | **新建** | +9 函数 |
| `app/api/music.ts` | 扩展 | +12 函数 |
| `app/api/index.ts` | 导出更新 | +36 导出 |
...
| 优先级 | 模块 | 接口数量 | 预估工时 | 状态 |
|--------|------|---------|---------|:---:|
| 🔴 **P0-高** | 用户体系增强 | ~24 | 3-5天 | ✅ 已完成 (24/24) |
| 🔴 **P0-高** | 歌手模块增强 | ~10 | 2-3天 | ✅ 已完成 (10/10) |
| 🟡 **P1-中** | 歌单模块增强 | ~16 | 3-4天 | ✅ 已完成 (16/16) |
| 🟡 **P1-中** | 社区/社交功能 | ~19 | 5-7天 | ⬜ 待开发 |
| 🟡 **P1-中** | 相似推荐 | ~5 | 1-2天 | ✅ 已完成 (5/5) |
| 🟡 **P1-中** | 每日推荐增强 | ~8 | 2-3天 | ✅ 已完成 (8/8) |
| 🟢 **P2-低** | 评论增强 | ~8 | 2-3天 | ✅ 已完成 (8/8) |
...
| 🟢 **P2-低** | 高级功能 | ~36 | 5-8天 | ✅ 已完成 (36/36) |
| ⚪ **P3-专业** | UGC/百科/音乐人 | ~14 | 3-5天 | ⬜ 待开发 |
| 🔵 **P3-增值** | 数字专辑 | ~8 | 2-3天 | ⬜ 待开发 |
| 🔵 **P3-数据** | 听歌足迹 | ~5 | 1-2天 | ⬜ 待开发 |
| 🔵 **P3-其他** | 杂项功能 | ~28 | 5-8天 | ⬜ 待开发 |

> **进度**：7/18 模块已完成（11.1 ✅ + 11.2 ✅ + 11.3 ✅ + 11.5 ✅ + 11.6 ✅ + 11.7 ✅ + 11.14 ✅），共完成 107/107 项。P0 + P1 全部模块已收官。
> **当前版本可打包发布**。剩余 11 个模块（11.4 社区/11.8 专辑/11.9 MV增强/11.10 电台/11.11 视频/11.12 云盘/11.13 VIP/11.15 UGC/11.16 数字专辑/11.17 足迹/11.18 杂项）后续按需迭代。
...
### 12.6 已知遗留（低优先级 — 有意设计，无需处理）

| 项目 | 说明 | 结论 |
|------|------|------|
| `next()`/`ManualNext` 顺序模式末尾行为 | 绕回第一首 vs `playlistStore.nextPlay()` 停止，行为与桌面端一致 | ✅ 有意保持 |
| RemotePlay/RemotePause 双重监听 | `playbackService.ts` 和 `trackPlayerService.ts` 各注册一份，无功能影响（简单 play/pause） | ✅ 无害，各有日志前缀区分 |
| `doPlaySong` UI 状态更新早于播放成功 | `setIsPlay(true)` 在 `tpPlaySong()` 之前，极端网络延迟下可能短暂显示错误状态 | ✅ 有意设计（优先响应 UI，注释已标注），极端情况概率极低 |
